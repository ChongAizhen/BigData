...
//    val ss = SparkSession.builder().appName("Spark In Action").master("local").getOrCreate()
    ...
    val conf = new SparkConf().setAppName("TrainNaiveBayes").setMaster("local")
    ...
    var srcRDD = sc.textFile("/home/user/IdeaProjects/github/BigData/data/mllib/sougou/train/").map {
      x =>
        var data = x.split(",")
    ...
    model.save(sc, "/home/user/IdeaProjects/github/BigData/data/mllib/sougou/model")
  }
}
